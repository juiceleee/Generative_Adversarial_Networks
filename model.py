...
# for Discriminator
W1_d = tf.get_variable("W1", shape = [3,3,1,32], stddev=0.01, initializer = tf.contrib.layers.xavier_initializer())
W2_d = tf.get_variable("W2", shape = [3,3,32,64], stddev=0.01, initializer = tf.contrib.layers.xavier_initializer())
...
# for Generator
W1_g = tf.get_variable("W1", shape = [noise_n,256], stddev=0.01, initializer = tf.contrib.layers.xavier_initializer())
b1_g = tf.Variable(tf.random_normal([256]))
W2_g = tf.get_variable("W2", shape = [256,512], stddev=0.01, initializer = tf.contrib.layers.xavier_initializer())
b2_g = tf.Variable(tf.random_normal([512]))
W3_g = tf.get_variable("W3", shape = [512,784], stddev=0.01, initializer = tf.contrib.layers.xavier_initializer())
